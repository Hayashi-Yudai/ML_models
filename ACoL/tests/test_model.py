    ...
    def test_shapes(self):
        ...
        with tf.Session() as sess:
            ...
            backborn_shape = sess.run(
                self.acol.conv5_3, feed_dict={self.acol.X: self.img}
            ).shape

            # Branch A
            flattenA_shape = sess.run(
                self.acol.flattenA, feed_dict={self.acol.X: self.img}
            ).shape
            feature_map_A_shape = sess.run(
                self.acol.feature_map_A, feed_dict={self.acol.X: self.img}
            ).shape

            # Branch B
            flattenB_shape = sess.run(
                self.acol.flattenB, feed_dict={self.acol.X: self.img}
            ).shape
            feature_map_B_shape = sess.run(
                self.acol.feature_map_B, feed_dict={self.acol.X: self.img}
            ).shape

            # Output
            output_shape = sess.run(
                self.acol.output, feed_dict={self.acol.X: self.img}
            ).shape
