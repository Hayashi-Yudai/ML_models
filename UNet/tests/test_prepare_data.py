...
class TestLoadDataset(unittest.TestCase):
    ...
    def setUp(self):
        self.train_data, self.validation_data = prepare_data.load_data(
            self.IMAGE_DIR, self.SEG_DIR, n_class=2, train_val_rate=0.9
        )
    ...
    def test_dataset_size(self):
        ...
        self.assertEqual(
            self.validation_data[0][0].shape, (128, 128, 3)
        )  # validation data
        self.assertEqual(
            self.validation_data[1][0].shape, (128, 128, 2)
        )  # validation data
    ...
    def test_generate_test_dataset(self):
        test_data, _ = prepare_data.load_data(
            self.IMAGE_DIR, None, n_class=2, train_val_rate=0.9
        )
